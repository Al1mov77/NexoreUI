...
// 2. We need to inject the mega/pro components into the correct existing sections.
// Let's replace the entire section for Button with a new one that includes the mega buttons.
// To do this reliably, we can inject them right before the closing div of the section.
...
// Remove the hardcoded Mega and Pro pack sections first
...
removeSection('{/* Mega Pack Showcase */}', '{/* Mega Pack Showcase */}');
removeSection('{/* Pro Pack Showcase */}', '{/* Pro Pack Showcase */}');

// Inject the components into existing sections
for (const [id, content] of Object.entries(injections)) {
  const regex = new RegExp('(id="' + id + '" className={[^{}]+}>[\\s\\S]*?)(</section>|</div>\\s*</div>\\s*<div id=)');
  const match = code.match(regex);
  if (match) {
    const updatedSection = match[1] + content + '\\n              ' + match[2];
    code = code.replace(match[0], updatedSection);
  } else {
    console.log("Could not find section for " + id);
  }
}
