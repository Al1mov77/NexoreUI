...
function parseDurationSecondsHelper(str: string): number {
  if (!str) return 0;
  let total = 0;
...
export function parseTelegramReports(rawText: string): SessionRecord[] {
  ...
  for (const block of blocks) {
    ...
    const sessionLifetime = lifetimeMatch ? parseDurationSecondsHelper(lifetimeMatch[1]) : 0;
    const activeTime = activeMatch ? parseDurationSecondsHelper(activeMatch[1]) : 0;
