...
export function generateHTMLCode(elements: NexoreMakeElement[], settings: CanvasSettings): string {
  ...
  const elementsHTML = sorted.map((el, idx) => {
    ...
    if (el.type === 'switch') {
      return `    <div class="nexore-switch ${className}${el.disabled ? ' disabled' : ''}">\n      <div class="nexore-switch-track${!el.checked ? ' off' : ''}">\n        <div class="nexore-switch-thumb${!el.checked ? ' off' : ''}"></div>\n      </div>\n      <span class="nexore-switch-label">${el.content || 'Switch'}</span>\n    </div>`;
    }
    if (el.type === 'checkbox') {
...
export function generateVueCode(elements: NexoreMakeElement[], settings: CanvasSettings): string {
  ...
  const elementsHTML = sorted.map((el, idx) => {
    ...
    if (el.type === 'switch') {
      return `    <div class="nexore-switch ${className}${el.disabled ? ' disabled' : ''}" ${!el.disabled ? `@click="switch${idx} = !switch${idx}"` : ''}>\n      <div :class="['nexore-switch-track', !switch${idx} && 'off']">\n        <div :class="['nexore-switch-thumb', !switch${idx} && 'off']"></div>\n      </div>\n      <span class="nexore-switch-label">${el.content || 'Switch'}</span>\n    </div>`;
    }
    if (el.type === 'checkbox') {
...
export function generateSvelteCode(elements: NexoreMakeElement[], settings: CanvasSettings): string {
  ...
  const elementsHTML = sorted.map((el) => {
    ...
    if (el.type === 'progress') {
      const pVal = Math.min(100, Math.max(0, parseInt(String(el.value ?? 60), 10) || 0));
      return `  <div ${styleAttr} class="progress-wrapper" style="display: flex; flex-direction: column; gap: 8px;">\n${el.label ? `    <span style="font-size: 12px; font-family: sans-serif; color: inherit;">${el.label}</span>\n` : ''}    <div class="progress-bar" style="background: #27272a; border-radius: 9999px; padding: 2px; width: 100%;">\n      <div style="width: ${pVal}%; height: 100%; background: #8b5cf6; border-radius: 9999px;"></div>\n    </div>\n  </div>`;
    }
    if (el.type === 'switch') {
...
export function generateAngularCode(elements: NexoreMakeElement[], settings: CanvasSettings): string {
  ...
  const elementsHTML = sorted.map((el) => {
    ...
    if (el.type === 'progress') {
      const pVal = Math.min(100, Math.max(0, parseInt(String(el.value ?? 60), 10) || 0));
      return `    <div ${styleAttr} class="ng-progress-wrapper" style="display: flex; flex-direction: column; gap: 8px;">\n${el.label ? `      <span style="font-size: 12px; font-family: sans-serif; color: inherit;">${el.label}</span>\n` : ''}      <div class="ng-progress-bar" style="background: #27272a; border-radius: 9999px; padding: 2px; width: 100%;">\n        <div style="width: ${pVal}%; height: 100%; background: #8b5cf6; border-radius: 9999px;"></div>\n      </div>\n    </div>`;
    }
    if (el.type === 'switch') {
